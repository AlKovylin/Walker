//Версия для работы с двигателями переменного тока
//Задний двигатель//PioDa -> DO 0,1; AO 0//COM1
//ПЛевый двигатель//PioDa -> DO 2,3; AO 1//COM2
//Правый двигатель//PioDa -> DO 4,5; AO 2//COM3
...
namespace DllFunc
{
    public class FuncForManager
    {
        ...
        private static Math_hodunki Match_MP = new Math_hodunki();
        ...
        private static AO_DO_DI PioDa = new AO_DO_DI();

        private FunkSettings FunkSett = new FunkSettings();
        private SetDevice[] deviceMass = new SetDevice[5];//имя порта, значение датчика на момент калибровки, сопоставленное устройство (NameDevice)

        private System.Timers.Timer timerALARM = new System.Timers.Timer();//таймер для отслеживания нажатия аварийной кнопки
        ...
        private bool flagRedLed = false;//флаг для моргания красным
        private Modes Mode = Modes.operating_mode;//0 - рабочий режим, 1 - режим настройки
        private double Ret_kren, Ret_tangag, Ret_height;
        private bool flagALARM = false;//внешний сигнал аварии
        private bool flagAlarmButton = false;//состояние аварийной кнопки
        ...
        public void PL_Start()
        {
            ...
            try
            {
                Move_ = new Moving(COM1, COM2, COM3, PioDa, deviceMass, Sett);
            ...
            PioDa.AlarmButtonPress += PioDa_AlarmButtonPress;//подписка на событие нажатия кнопки аварии
            PioDa.AlarmButtonNOPress += PioDa_AlarmButtonNOPress;//подписка на событие возврата кнопки аварии

            timerALARM.Interval = 500;
            timerALARM.AutoReset = true;
            timerALARM.Enabled = false;
            timerALARM.Elapsed += OnTimedEventAlarm;
            timerALARM.Start();
        ...
        /// ПОПРАВИТЬ!!!!!!!!!!!!!!!!!!!!!!!!
        public bool PL_Alarm(bool data)//условно готово
        {
            try
            {
                if (!data && !flagAlarmButton)//если нет внешнего сигнала аварии, если не нажата кнопка аварии
                {
                    flagALARM = data;
                    timerALARM.Enabled = false;//отключаем моргание красным
                    Leds.OFF_LED();
                    Leds.GreenON();
                }
                if (data)//тревога, аварийный останов
                {
                    Move_.TokenCancel();//останавливаем предыдущую операцию
                    StopMotors();
                    Leds.OFF_LED();//гасим все светодиоды
                    timerALARM.Enabled = true;//запускаем моргание красным
                    flagALARM = data;
                }
                return true;
            }
        ...
        /// <param name="e"></param>
        private void OnTimedEventAlarm(Object source, System.Timers.ElapsedEventArgs e)//Timer tick
        {
            if (flagRedLed) { Leds.RedON(); flagRedLed = false; }
            else { Leds.RedOFF(); flagRedLed = true; }
        }

        /// <summary>
        /// Обработка события нажатия кнопки аварии
        /// </summary>
        private void PioDa_AlarmButtonPress()
        {
            Leds.GreenOFF();
            Leds.BlueOFF();
            Move_.TokenCancel();//останавливаем предыдущую операцию
            timerALARM.Enabled = true;//запускаем мигание
            flagAlarmButton = true;
        }
        
        /// <summary>
        /// Обработка события отключения кнопки аварии
        /// </summary>
        private void PioDa_AlarmButtonNOPress()
        {
            flagAlarmButton = false;
        ...
        /// </summary>
        /// <returns></returns>
        public double PL_Read_height()//УСЛОВНО ГОТОВО
        {
            HKT();
            return Ret_height;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public double PL_Read_angle_kren()//УСЛОВНО ГОТОВО
        {
            HKT();
            return Ret_kren;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public double PL_Read_angle_tangag()//УСЛОВНО ГОТОВО
        {
            HKT();
            return Ret_tangag;           
        }
        ...
        /// </summary>
        private void HKT()//Функции использующие эту функцию оснащены заглушками до решения задачи.
        {
        ...
        public void PL_Move_height(double z, double s)//условно готово
        {
            if (!flagALARM)
            {
                try
                {
                    if (z <= PhysicalChar.Max_height && z >= PhysicalChar.Min_height)
                    {
